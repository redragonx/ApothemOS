...
*  system, that simply turns the OK LED on and off repeatedly, but now using the  *  EABI standard, and procedure calls. Also new is an appreciation of the IVT.
...
/* NEW
* Use our new SetGpio makefunction to set GPIO 16 base on the current bit in the 
...
.align 2 	/*.align 2 which means that this data will definitely be placed 		 * at an address which is a multiple of 22 = 4.*/

		/*It is really important to do this, because the ldr instruction 			 * we used to read memory only works at addresses that are 			 * multiples of 4.*/	
pattern:
.int 0b11111111101010100010001000101010 /*11111111101010100010001000101010(base2) 						 *will be placed into the output*/
